...
func NewAppLogger(cfg config.ILoggerConfig) *appLogger {
	return &appLogger{level: cfg.GetLogLevel(), devMode: cfg.IsDevelopment(), encoding: cfg.GetEncoder()}
}

// For mapping sdk-config logger to email_service logger levels
...
func (l *appLogger) InitLogger() {
	...
	case "no-caller-console":
		{
			encoderCfg.EncodeLevel = zapcore.CapitalColorLevelEncoder
			encoderCfg.EncodeCaller = NoCallerEncoder
			encoderCfg.ConsoleSeparator = " | "
			encoder = zapcore.NewConsoleEncoder(encoderCfg)
		}
	default:
...
func AppLogger(cfg config.IAppConfig) IAppLogger {
	al := NewAppLogger(cfg.GetLoggerConfig())
	al.InitLogger()
	al.WithName(cfg.GetServiceConfig().GetServiceName())
	return al
}
